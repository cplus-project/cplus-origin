...
/****** methods of DRIdentTable ******/

error drIdentTableInit(DRIdentTable* drid_table, IdentTable* id_table) {
    if (id_table == NULL) {
        return new_error("the identifier table can not be NULL.");
    }
    drid_table->id_table = id_table;
    drid_table->head     = NULL;
    drid_table->tail     = NULL;
    return NULL;
}

bool drIdentTableIsEmpty(DRIdentTable* drid_table) {
    return drid_table->head == NULL ? true : false;
}

void drIdentTableAdd(DRIdentTable* drid_table, char* drid_name) {
    DRIdentTableNode* create = (DRIdentTableNode*)mem_alloc(sizeof(DRIdentTableNode));
    create->drid_name = drid_name;
    create->next      = NULL;
    drid_table->head != NULL ? (drid_table->tail->next = create) : (drid_table->head = create);
    drid_table->tail  = create;
}

error drIdentTableResolve(DRIdentTable* drid_table) {
    // TODO: resolve every entry in the delay resolve identifiers table...
    return NULL;
}

void drIdentTableDestroy(DRIdentTable* drid_table) {
    DRIdentTableNode* del = NULL;
    for (;;) {
        if (drid_table->head == NULL) {
            drid_table->tail =  NULL;
            return;
        }
        del = drid_table->head;
        drid_table->head = drid_table->head->next;
        mem_free(del);
    }
}
